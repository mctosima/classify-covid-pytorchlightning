...
class OurModel(LightningModule):
    def __init__(self):
        ...
        self.train_set, self.val_set, self.test_set = random_split(self.dataset, [self.train_size, self.val_size, self.test_size])
    ...
    def train_dataloader(self):
        dl = DataLoader(self.train_set, batch_size=self.batch_size, shuffle=True, num_workers=self.num_workers)
        return dl
    ...
    def val_dataloader(self):
        dl = DataLoader(self.val_set, batch_size=self.batch_size, shuffle=False, num_workers=self.num_workers)
        return dl
    ...
    def test_dataloader(self):
        dl = DataLoader(self.test_set, batch_size=self.batch_size, shuffle=False, num_workers=self.num_workers)
        return dl
...
trainer = Trainer(devices=1,
                  ...
                  callbacks=[EarlyStopping(monitor='train/loss', patience=2, min_delta=0.01),
                            #  checkpoint_callback
                             ],
                  logger=wandb_logger,
