...
from torch.utils.data import DataLoader, Dataset, random_split, ConcatDataset, SubsetRandomSampler
...
class XrayVision(LightningModule):
    ...
    def train_dataloader(self):
        dl = DataLoader(DataReader(self.combined,aug),
    ...
    def test_dataloader(self):
        dl = DataLoader(DataReader(test_set),
                        batch_size=self.batch_size,
                        shuffle=False,
                        num_workers=self.num_workers,
                        pin_memory=True,
...
for fold,(train_idx, val_idx) in enumerate(kfold.split(combined_set)):
    ...
    checkpoint = ModelCheckpoint(monitor='val/loss', mode='min', save_top_k=1, filename=f'fold_{fold}_checkpoint.pth')
